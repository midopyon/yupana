...
function get_user_input() {
  ...
  _laser_iteration = parseFloat(
    document.getElementById("laser_iteration").value
  );

  _laser_thickness =
    parseFloat(document.getElementById("mat_thickness_input_lasercut").value) /
  ...
  _laser_weight =
    _laser_area *
    _laser_thickness *
    material_laser[_laser_material].density *
    _laser_iteration; //weight (kg) = volume (m3) * density (kg/m3)
  console.log("Weight: " + _laser_weight);
  if (document.querySelector("input[value=waste_percent]").checked) {
    let percent_waste =
      parseFloat(document.getElementById("waste_laser").value) / 100;
    _laser_waste = _laser_weight * percent_waste;
  } else {
    console.log("calc waste area");
    let _laser_waste_area = parseFloat(
  ...
  _laser_cut_time =
    parseFloat(document.getElementById("time").value) * 60 * _laser_iteration; //covert from min to sec
...
document
  .getElementById("btn_submit_laser")
  .addEventListener("click", start_graphing);
...
function lifecycle_calculation_laser() {
  ...
  _energy.mat_manufacturing =
    _laser_weight * material_laser[_laser_material]["emb_energy_avg"];
  _co2.mat_manufacturing =
    _laser_weight * material_laser[_laser_material]["co2_avg"];
  ...
  //fabrication
  _energy.fabrication =
    (_laser_cut_time * 0.85 * machine_energy[_laser_machine].cutting +
      _laser_cut_time * 0.15 * machine_energy[_laser_machine].stand_by +
      _laser_cut_time * 0.2 * machine_energy[_laser_machine].idle) /
    1000000;
...
let _laser_manu_exclamation = document.querySelector(
...
let _laser_transport_exclamation = document.querySelector(
...
function set_fabrication_laser() {
  ...
  if (_laser_machine == "Trotec") {
    ...
    text.appendChild(
      document.createTextNode(
        "Epilog Fusion Pro 32 uses 41% less power than Trotec Speedy 400, and 25% less power than Universal PLS6.75 in the cutting process. You can save energy by reducing laser cutting time, avoiding double cuts, optimizing the cutting area, and using the optimal cutting parameters per material. You can also save energy reducing idle time by turning off the machine when not in use, or sending jobs in series."
      )
    );
  } else {
    //Universal
    text.appendChild(
      document.createTextNode(
        "Universal PLS6.75 uses 25% more power than Epilog Fusion Pro 32, and 12% less than Trotec Speedy 400 in the cutting process. You can save energy by reducing laser cutting time, avoiding double cuts, optimizing the cutting area, and using the optimal cutting parameters per material. You can also save energy reducing idle time by turning off the machine when not in use, or sending jobs in series."
      )
    );
  }
...
let _laser_fabrication_exclamation = document.querySelector(
...
let _laser_end_life_exclamation = document.querySelector(
