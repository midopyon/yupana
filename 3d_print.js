function Object_3dprint(
  ...
  this.prototyping_weight = prototyping_weight;
  this.prototyping_waste = prototyping_waste;
...
function start_the_magic_calculate_mode() {
  //save the contents of the page to an object, calculate values and show graph

  // do the same as start the magic
  refresh_user_input();
  ...
  DrawGoogleCharts([results_energy]);
}
...
function start_the_magic() {
  ...
  if (results_energy_ar.length == 0) {
    original = {
      ...
      prototype_weight: Math.round(
        (_3dprint_weight * 1000) / _3dprint_iteration
      ),
      prototype_waste: Math.round(
        (_3dprint_support * 1000) / _3dprint_iteration
      ),
